# Linux_Configuration
#Prepared by MADISETTY PRATHYUSHA
...
- The web application is my [Item Catalog project](https://github.com/MPrathyusha21/catalog) created earlier in this Nanodegree program.
- The database server is [PostgreSQL](https://www.postgresql.org/).

You can visit http://54.198.42.182.xip.io/ or http://ec2-54-198-42-182.compute-1.amazonaws.com/ for the website deployed.
...
### Step 1: Start a new Ubuntu Linux server instance on Amazon EC2 
  ...
  - Check for instance IPv4 public IP - 54.198.42.182
  - we can download a .pem file and connect with following command
    ```
    ssh -i prathyusham.pem ubuntu@54.198.42.182
    ``` 
...
- To check port 2200 wether working or not by `ssh -i prathyusham.pem -p 2200 ubuntu@54.198.42.182` 
...
### Step 7: Create an SSH key pair for `grader`
  ...
  - Restart SSH: `sudo service ssh restart`
  - On the local machine, cheking if the grader account working or not by running this command : `ssh -i prathyusham.pem -p 2200 grader@54.198.42.182`.
...
`sudo git clone https://github.com/MPrathyusha21/catalog.git`.
- Change the ownership of the `catalog` directory to `grader` using: `sudo chown -R grader:grader catalog/`.
- Change to the `/var/www/catalog/catalog` directory.
- Rename the `main_file.py` file to `__init__.py` using: `mv main.py __init__.py`.
- We need to change sqlite to postgresql create_engine in `__init__.py`,`Setup_file.py` and `Init_file.py`,
   ```
...
### Step 13.2: Set up and enable a virtual host
  ...
  <VirtualHost *:80>
      ...
      ServerAlias ec2-54-198-42-182.compute-1.amazonaws.com
      ServerAdmin ubuntu@54.198.42.182
...
- Open your browser to http://54.198.42.182.xip.io or http://ec2-54-198-42-182.compute-1.amazonaws.com.
